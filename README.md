...
<!-- BEGIN_MODULE_INDEX -->
* `asg` – Creates:
* `ec2_instance` – Creates a single EC2 instance with secure defaults:
* `efs` – Creates an EFS file system and one mount target per subnet.
* `iam_role` – Creates an IAM Role with:
* `nlb` – Creates a Network Load Balancer (NLB) with:
* `s3` – Creates an S3 bucket with production defaults:
* `security_group` – Creates an AWS Security Group and a set of ingress/egress rules.
<!-- END_MODULE_INDEX -->
